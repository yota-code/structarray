...
class DataRebin(DataGeneric):
	...
	def __init__(self, data_pth: Path, meta=None):
		...
		match meta:
			case Path() | str():
				self.meta = structarray.rebin.meta._cached_meta[
					Path(meta).resolve(strict=True)
				]
	...
	def _load(self):
		...
		self.block_len = (
			(((self.meta.sizeof // self.block_boundary) + 1) * self.block_boundary)
			if (self.meta.sizeof % self.block_boundary) != 0
			else self.meta.sizeof
		)
		# nombre de blocs
		...
		self.data_len = (
			(file_len // self.block_nbr) * self.block_nbr
		...
		print(
			f" => file of {self.data_len} bytes"
			+ (
				f" \x1b[33mtruncated\x1b[0m to {self.data_len}"
				if self.data_len != file_len
				else ""
			)
			+ f" or {self.block_nbr} blocks of {self.block_len} bytes"
		)
	...
	def extract_structured_array(self, *c_lst):
		...
		d_lst = [
			(f"{i:05d}_{c.split('.')[-1]}", ntype_map[self.meta.get_type(c)])
